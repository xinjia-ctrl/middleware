...
基于注解 + AOP 的 Spring Boot 幂等组件，防止重复提交，支持结果缓存和分布式部署，开箱即用。

## 功能特性

- 注解驱动：`@Idempotent` 声明式幂等控制
- 结果缓存：重复请求直接返回第一次的执行结果，而非抛异常
- 双重存储：自动检测 Redis 依赖，有 Redis 走分布式去重，否则降级为本地内存
- SpEL 表达式：灵活提取幂等 Key（请求头、参数、用户 ID 等）
- 异常处理：重复请求统一返回 409 Conflict，结果缓存模式下直达缓存结果
- Spring Boot 自动装配：引入依赖即用，零配置
...
public class ApiController {
    ...
    @PostMapping("/submit")
    @Idempotent(key = "#request.getHeader('Idempotent-Key')", ttl = 2, timeUnit = TimeUnit.HOURS, cacheResult = true)
...
| `cacheResult` | `boolean` | `false` | 是否缓存执行结果，重复请求时直接返回 |
...
- `#userId` — 按参数值去重
- `#dto.orderNo` — 按对象属性去重
- `#userId + ':' + #dto.type` — 组合 Key

不配置 key 时，默认以方法签名作为 Key（全局限流）。

### 存储策略

引入 `spring-boot-starter-data-redis` 且配置 Redis 连接后自动启用分布式存储，否则回退到本地内存：
...
| 存储 | 原理 | 适用场景 |
|------|------|----------|
| `InMemoryIdempotentStorage` | `ConcurrentHashMap` | 单实例，开发/测试 |
| `RedisIdempotentStorage` | `SET NX PX` | 多实例生产部署 |
...
重复请求:  trySave 失败 → getResult(key) → 有缓存 → 直接返回
                                      → 无缓存 → 抛 DuplicateRequestException
