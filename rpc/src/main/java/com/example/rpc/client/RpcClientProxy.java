...
public class RpcClientProxy {
    ...
    private final RpcFallback fallback;
    ...
    public RpcClientProxy(RpcClient rpcClient, String host, int port) {
        this(rpcClient, host, port, 2, null);
    }

    public RpcClientProxy(RpcClient rpcClient, String host, int port, int retryCount, RpcFallback fallback) {
    ...
    public RpcClientProxy(RpcClient rpcClient, ServiceDiscovery discovery) {
        this(rpcClient, discovery, 2, null);
    }

    public RpcClientProxy(RpcClient rpcClient, ServiceDiscovery discovery, int retryCount, RpcFallback fallback) {
        this.rpcClient = rpcClient;
            ...
            for (int i = 0; i <= retryCount; i++) {
                ...
                try {
                    ...
                    if (response.isSuccess()) {
                        if (fallback != null) {
                            fallback.onSuccess(interfaceName, method, response.getData());
                        }
                    ...
                    if (i >= retryCount) {
                        if (fallback != null) {
                            return fallback.fallback(interfaceName, method, args, lastException);
                        }
                        throw lastException;
                    }
                }
