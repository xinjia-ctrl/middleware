...
基于注解 + AOP 的 Spring Boot 缓存一致性中间件，解决数据库与缓存之间的数据一致性问题。

## 功能特性

- **两种一致性策略**：先更新后删缓存（EVICT_AFTER）和延迟双删（DOUBLE_DELETE）
- **两种缓存操作**：淘汰缓存（EVICT）和写入缓存（PUT）
- PUT 模式支持 JSON 序列化，自动缓存方法返回值
- 存储降级：自动检测 Redis 依赖，无 Redis 时降级为本地内存
- 延迟双删：配合定时任务延迟二次删除，解决主从复制并发读不一致
- SpEL 表达式动态提取缓存 Key
...
| `delayMillis` | `long` | `500` | 延迟双删等待时间（毫秒） |

### 一致性策略

| 策略 | 流程 | 适用场景 |
|------|------|----------|
| `EVICT_AFTER` | 执行方法 → 删除/写入缓存 | 简单场景 |
